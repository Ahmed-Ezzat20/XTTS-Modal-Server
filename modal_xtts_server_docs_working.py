...
app = modal.App("xtts-server-docs-working", image=image)
...
# Global variable to store the model (will be loaded once)
model_instance = None
model_config = None
model_device = None
...
def load_xtts_model():
    """Load the XTTS model when called"""
    global model_instance, model_config, model_device
...
async def generate_tts_modal(
    ...
    if speaker_id:
        speaker_path = f"/speakers/{speaker_id}.wav"
        if not os.path.exists(speaker_path):
            raise HTTPException(
                status_code=404, detail="Speaker ID not found. Please register first."
            )
    elif speaker_wav_url or speaker_wav_base64:
        # Create temporary speaker from provided audio
        result = await register_speaker_modal(speaker_wav_url, speaker_wav_base64)
        speaker_path = result["path"]
    else:
        raise HTTPException(
    ...
    try:
        ...
        gpt_cond_latent, speaker_embedding = (
            model_instance.get_conditioning_latents(audio_path=[speaker_path])
        )
        
        # Generate speech
        with torch.no_grad():
            out = model_instance.inference(
                text=text,
                language=language,
...
async def root():
    ...
    return ServiceInfo(
        status="ok",
        service="XTTS v2 Inference API on Modal (Working with Docs)",
        ...
        version="working-with-full-documentation"
    )
...
async def tts(req: TTSRequest):
    ...
    """
    try:
        audio_bytes = await generate_tts_modal.remote.aio(
            text=req.text,
...
if __name__ == "__main__":
    ...
    print("XTTS Modal server with working documentation ready for deployment!")
    print("Deploy with: modal deploy modal_xtts_server_docs_working.py")
